...
public interface GunPartRepo extends JpaRepository<GunPart, Long> {
    ...
    @Query(value =
            "SELECT gp.gun_part_id, gp.gun_part_image_url, gp.width, gp.x, gp.y, gp.product_id, gp.thumbnail_image, p.name, p.product_image_url, p.type FROM gun_part as gp JOIN product p on p.product_id = gp.product_id WHERE gun_part_id = ?1"
    , nativeQuery = true)
